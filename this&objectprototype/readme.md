...
JavaScript没有类，但是可以近似实现很多类的功能
类继承意味着拷贝。
但是JS中的对象机制不会自动的执行拷贝行为，对象不是拷贝到另一个对象中，而是连接在一起
所以需要在js中模拟这种缺失的类的拷贝：mixins
对当前对象不存在的属性进行拷贝
多态，显式假想多态
...
当 foo 不直接存在 于 myObject，但 存在 于 myObject 的 [[Prototype]] 链的更高层时：
